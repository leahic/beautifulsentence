...
def train_predict(programpath , options , dirpath , traindat , testdat , modeldat , predictdat):
	...
	command = [ programpath + 'svm-train' ] + options + [dirpath + traindat] + [dirpath + modeldat]
	command = ' '.join(command)
	print command
	os.system( command )
	print 'trian over'

	command = [ programpath + 'svm-predict' ] + ['-b' , '1'] +  [dirpath + testdat] + [dirpath + modeldat] + [dirpath + predictdat]
	command = ' '.join(command)
	print command
	os.system( command )
	print 'predict over'
...
def MAP(dirpath , predictdat):
	...
	for index , line in enumerate(f):
		if index > 0:
			targetvecs.append( [ eval(x) for x in line.split() ] )
	f.close()
